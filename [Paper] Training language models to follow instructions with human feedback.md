...
### 정렬과 인간 피드백을 통한 학습 연구
우리는 모델을 인간의 의도와 정렬하기 위한 이전 기술들, 특히 인간 피드백을 통한 강화학습(RLHF)을 기반으로 한다. 원래 Atari 게임에서 간단한 로봇을 훈련시키기 위해 개발되었지만(Christiano 외., 2017; Ibarz 외., 2018), 최근에는 텍스트 요약(Ziegler 외., 2019; Stiennon 외., 2020; Böhm 외., 2019; Wu 외., 2021)을 위한 언어 모델의 미세조정에 적용되었다. 이 연구는 대화(Jaques 외., 2019; Yi 외., 2019; Hancock 외., 2019), 번역(Kreutzer 외., 2018; Bahdanau 외., 2016), 의미 분석(Lawrence and Riezler, 2018), 스토리 생성(Zhou and Xu, 2020), 리뷰 생성(Cho 외., 2018), 증거 추출(Perez 외., 2019)과 같은 영역에서 보상으로 인간 피드백을 사용한 유사 연구의 영향을 받았다. Madaan 외.(2022)는 GPT-3의 성능을 향상시키기 위해 작성된 인간 피드백을 사용하여 프롬프트를 보강했다. 규범적 사전 확률을 사용하여 텍스트 기반 환경에서 RL로 에이전트를 정렬하는 연구도 있었다(Nahian 외., 2021). 우리의 연구는 RLHF를 폭넓은 언어 작업 분포에 대한 언어 모델 정렬에 직접 적용한 것으로 볼 수 있다.

언어 모델의 정렬이 무엇을 의미하는가에 대한 질문도 최근 주목을 받고 있다(Gabriel, 2020). Kenton 외.(2021)는 잘못된 정렬로 인한 언어 모델의 행동 문제들을 분류했는데, 여기에는 유해한 콘텐츠 생성과 잘못 명시된 목표의 악용이 포함된다. 동시에 진행된 연구에서 Askell 외.(2021)는 정렬 연구를 위한 테스트베드로 언어 비서를 제안하고, 몇 가지 기본 베이스라인과 그들의 확장 속성을 연구했다.

지시사항을 따르도록 언어 모델 훈련하기: 우리의 연구는 또한 언어 모델의 교차 작업 일반화에 관한 연구와도 관련이 있다. 여기서 언어 모델들은 광범위한 공개 NLP 데이터셋(보통 적절한 지시사항이 앞에 붙음)에 대해 미세조정되고 다른 작업 세트에서 평가된다. 이 분야에서 많은 연구가 있었고(Yi 외., 2019; Mishra 외., 2021; Wei 외., 2021; Khashabi 외., 2020; Sanh 외., 2021; Aribandi 외., 2021), 이들은 훈련과 평가 데이터, 지시사항의 형식화, 사전 훈련된 모델의 크기, 기타 실험 세부사항에서 차이가 있다. 이 연구들에서 일관된 발견은 NLP 작업들에 대해 지시사항과 함께 언어 모델을 미세조정하면 제로샷 및 퓨샷 환경 모두에서 보류된 작업들에 대한 후속 성능이 향상된다는 것이다.

또한 내비게이션을 위한 지시사항 따르기에 대한 관련 연구도 있다. 여기서 모델들은 시뮬레이션 환경에서 자연어 지시사항을 사용해 탐색하도록 훈련된다(Bahdanau 외., 2018; Abramson 외., 2020; Zhao 외., 2021).

### 언어 모델의 위험 평가

언어 모델의 행동을 수정하는 목표는 이러한 모델들이 실제 세계에 배포될 때의 위험을 완화하는 것이다. 이러한 위험들은 광범위하게 문서화되어 왔다(Bender 외., 2021; Bommasani 외., 2021; Kenton 외., 2021; Weidinger 외., 2021; Tamkin 외., 2021). 언어 모델은 편향된 출력을 생성하고(Dhamala 외., 2021; Liang 외., 2021; Manela 외., 2021; Caliskan 외., 2017; Kirk 외., 2021), **개인 정보를 유출**하며(Carlini 외., 2021), 잘못된 정보를 생성하고(Solaiman 외., 2019; Buchanan 외., 2021), 악의적으로 사용될 수 있다. 상세한 검토를 위해 Weidinger 외.(2021)를 참조하기 바란다. 특정 영역에서 언어 모델을 배포하면 새로운 위험과 과제가 발생하는데, 예를 들어 대화 시스템에서 이러한 현상이 나타난다(Henderson 외., 2018; Xu 외., 2020; Dinan 외., 2019b). 이러한 위험들을 구체적으로 평가하기 위한 벤치마크를 구축하려는 초기 단계의 성장하는 분야가 있는데, 특히 유해성(Gehman 외., 2020), 고정관념(Nadeem 외., 2020), 사회적 편향(Dhamala 외., 2021; Nangia 외., 2020; Rudinger 외., 2018) 주변에서 이루어지고 있다. 이러한 문제들에서 상당한 진전을 이루기는 어려운데, 언어 모델 행동에 대한 의도하지 않은 개입이 부작용을 가질 수 있기 때문이다(Welbl 외., 2021; Blodgett 외., 2020). 예를 들어, 언어 모델의 유해성을 줄이려는 노력은 훈련 데이터의 편향된 상관관계로 인해 과소 대표된 그룹의 텍스트를 모델링하는 능력을 감소시킬 수 있다(Xu 외., 2021).
